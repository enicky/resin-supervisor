...
module.exports = class ServiceManager extends EventEmitter
	...
	reportNewStatus: (containerId, service, status) =>
		@reportChange(containerId, _.merge({ status }, _.pick(service, [ 'imageId', 'appId', 'releaseId' ])))
	...
	getStatus: =>
		...
		.then (services) =>
			...
			_.forEach services, (service) ->
				status[service.containerId] ?= _.pick(service, [ 'appId', 'imageId', 'status', 'releaseId' ])
